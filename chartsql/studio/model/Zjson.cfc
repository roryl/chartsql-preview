component {
	...
	public function persist(required string path){
		var map = this.toFlatMap();
		writeDump(now());
		//Overwrite the file with the new values
		fileWrite(path, "");
		var file = fileOpen(path, "write");
		for(var key in map){
			var value = key & " = " & map[key];
			fileWriteLine(file, value);
		}
		fileClose(file);
	}

	static function read(required string path){
	...
	public function toFlatMap(required any data=getValues(), prefix="", ignore=[]){
    	var prefix = arguments.prefix;
		var pile = structNew("ordered");
