		...
		<div class="btn-toolbar" role="toolbar" aria-label="Toolbar with button groups">
			<div class="btn-group" role="group" aria-label="First group">
				...
				{{#each view_state.datetime_slicer_links}}
					<button zero-icon="true" onclick="document.getElementById('datetimeSlicer{{key}}').click();" type="button" class="btn {{#if isActive}}active{{/if}}" data-bs-toggle="tooltip" zero-icon="false" data-bs-placement="bottom" title="From 1 day ago">{{key}}</button>
