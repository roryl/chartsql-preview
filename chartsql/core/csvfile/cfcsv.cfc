<cfcomponent output="no" hint="csvToArray function by Ben Nadel: http://www.bennadel.com/index.cfm?event=blog.view&id=2041">
	<!--- --------------------------------------------------------------------------------------- ----
        ...
        https://github.com/paulklinkenberg/lucee-cfcsv/blob/master/source/cfml/tags/cfcsv/cfcsv.cfc
				...
				<cfif local.firstRow>
					<cfif arguments.hasColumnNames>
						<cfif len(trim(local.fieldValue))>
							<cfscript>
								//2022-01-08: Replace periods with underscores
								local.fieldValue = replace(local.fieldValue,".","_","all");
							</cfscript>
